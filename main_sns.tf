resource "aws_kms_key" "sns" {
  count                    = (var.sns_encryption == true) ? 1 : 0
  description              = "${var.tenant}-${var.name}-sns-kms-${var.environment}"
...
POLICY
  ...
  tags = {
    Name        = "${var.tenant}-${var.name}-sns-kms-${var.environment}"
    Tenant      = var.tenant
    Project     = var.name
...
resource "aws_kms_alias" "sns" {
  count         = (var.sns_encryption == true) ? 1 : 0
  name          = "alias/${var.tenant}/${var.name}/sns/${var.environment}"
